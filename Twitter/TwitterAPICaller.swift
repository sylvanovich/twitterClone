...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func favoriteTweet(tweetId:String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
    ...
    func unfavoriteTweet(tweetId:String, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/unfavorites/destroy.json"
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
